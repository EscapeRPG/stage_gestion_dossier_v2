<article>
    <h2 class="closed">
        Commentaires
        <button type="button" id="comment-btn">+</button>
    </h2>

    <div class="content" id="commentaire">
        <div class="commentaire-histo">
            <label for="commentaire-histo">Objet :</label>
            <input type="text" name="commentaire-histo" id="commentaire-histo"
                   placeholder="Commentaire succint (max: 60 caractères)" maxlength="60">
        </div>

        <textarea name="commentaire-detail-histo" rows="10" placeholder="Commentaire détaillé"></textarea>
    </div>
</article>
